...
Every lesson automatically ends with a closing sign-off ("وكده نكون خلّصنا
الدرس الأول بحمد الله" + `END OF LESSON NN`), generated by
`render_lesson_end()` in `build/build.py` — don't write it into the
markdown. Arabic ordinals live in the `AR_ORDINALS` map there; extend it
past lesson 30 when needed.
